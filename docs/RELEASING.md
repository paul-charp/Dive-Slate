...
The tree currently carries `versionCode = 20` / `versionName = "0.7.1"`, so the
release after this one is:

```kotlin
versionCode = 21        // must increase; this is what the updater compares
versionName = "0.7.2"   // must equal the tag without its leading v
```

Write what the release page should say, in `docs/release-notes/<tag>.md`:

```markdown
# Dive Slate 0.4.5 — what this one is
...
| Check | Why it exists |
|---|---|
| Tag matches `versionName` | Otherwise `v0.4.5` ships an APK whose start screen says 0.4.4 |
